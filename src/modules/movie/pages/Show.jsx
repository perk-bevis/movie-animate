...
// Component con cho việc hiển thị trạng thái loading
...
// Component chính: Show
const Show = ({ title: initialTitle }) => {
  // State để quản lý các bộ lọc
  const [filters, setFilters] = useState({
    type: "tv-shows", // Mặc định là phim bộ
    category: "",
    country: "",
    year: "",
    sort_field: "modified.time", // Mặc định sắp xếp theo thời gian cập nhật
  });
  ...
  // State để quản lý chế độ xem (grid/list)
  const [view, setView] = useState("grid"); // Mặc định là 'grid'

  // useEffect để lấy dữ liệu cho các dropdown filter (chỉ chạy 1 lần)
  useEffect(() => {
  ...
  // useEffect để lấy danh sách phim khi filter hoặc trang thay đổi
  useEffect(() => {
    const fetchMovies = async () => {
      setLoading(true);
      try {
        const API_URL = `https://phimapi.com/v1/api/danh-sach/${filters.type}`;
        
        const params = new URLSearchParams({
          page: currentPage,
          sort_field: filters.sort_field,
          limit: 20, // Số lượng item mỗi trang
        });
        ...
        setMovies([]); // Reset danh sách phim nếu có lỗi
      } finally {
        setLoading(false);
    ...
    fetchMovies();
  }, [filters, currentPage]); // Chạy lại khi filters hoặc currentPage thay đổi

  // Hàm xử lý khi người dùng thay đổi giá trị của filter
  ...
  // Hiển thị loading spinner nếu đang tải và chưa có phim nào
    ...
    <div className="bg-[#181818] text-white">
      <div className="p-4 sm:p-8 ">
        <div className="max-w-screen-xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
              ...
              <div className="flex items-center self-end h-[42px] justify-self-end">
                ...
                <div className="flex bg-gray-700 rounded-md p-1 gap-1">
                  <button onClick={() => setView('list')} className={`p-1.5 rounded ${view === 'list' ? 'text-white bg-blue-600' : 'text-gray-400 hover:text-white hover:bg-gray-600'}`}>
                  ...
                  <button onClick={() => setView('grid')} className={`p-1.5 rounded ${view === 'grid' ? 'text-white bg-blue-600' : 'text-gray-400 hover:text-white hover:bg-gray-600'}`}>
          ...
          {/* Hiển thị danh sách phim hoặc thông báo không tìm thấy */}
            ...
            <div className={
                ...
                {movies.map((movie) => (
                    view === 'grid' ? (
                        ...
                        // Giao diện List
                        <div key={movie._id} className="group cursor-pointer">
                             <Link to={`/phim/${movie.slug}`} className="flex gap-4 p-3 bg-[#212121] rounded-lg hover:bg-gray-800 transition-colors duration-300">
